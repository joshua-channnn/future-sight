# embedding_extractor_v17.py
"""
v17.2 Feature Extractor — Updated for 640-dim observations.

Changes from v17.1 (623 dims):
- Per-Pokemon float features: 30 -> 31 (+move_knowledge)
- Global features: 55 -> 60 (+5 bench move effectiveness)
- Total: 623 -> 640

The SharedPokemonEncoder input size changes: 182 -> 183
(one additional float feature per Pokemon block)
"""
...
class ActiveMatchupHead(nn.Module):
    ...
    def __init__(self, pokemon_repr_dim, move_embed, move_embed_dim=24, output_dim=128):
        super().__init__()
        self.move_embed = move_embed
        # Per move: embed(24) + 12 floats = 36. Total: 128*2 + 36*4 = 400
...
class V17FeatureExtractor(BaseFeaturesExtractor):
    """v17.2 Feature Extractor for 640-dim observations."""
    ...
    def __init__(self, observation_space, features_dim=256,
        ...
        self.pokemon_encoder = SharedPokemonEncoder(
            ...
            self.item_embed, self.type_embed,
            n_float_features=31,  # CHANGED from 30
        ...
        self.n_global_features = 60  # CHANGED from 55

        final_input = pokemon_repr_dim * 3 + self.n_global_features  # 128*3 + 60 = 444
    ...
    def forward(self, observations):
        ...
        # Active move details: 12 Pokemon * 44 = 528, then 48 floats, then 4 indices
        md_offset = 12 * PER  # 528 (was 516)
        mi_offset = md_offset + 48  # 576 (was 564)
        gl_offset = mi_offset + 4   # 580 (was 568)
        ...
        our_bench_pool = our_bench.max(dim=1).values
        opp_team_pool = torch.cat([opp_active.unsqueeze(1), opp_bench], dim=1).max(dim=1).values
