...
class ActiveMatchupHead(nn.Module):
    ...
    CHANGED from v17.3sp: move_floats_per_move is now 15 (was 12)
    due to categorical effectiveness encoding.
    """

    def __init__(self, pokemon_repr_dim, move_embed, move_embed_dim=24,
                 move_floats_per_move=15, output_dim=128):
        super().__init__()
        self.move_embed = move_embed
        self.move_floats_per_move = move_floats_per_move
        per_move = move_embed_dim + move_floats_per_move
...
class V18FeatureExtractor(BaseFeaturesExtractor):
    """
    v18 Feature Extractor with enhanced battle context.

    Observation layout (677 dims):
      Per-pokemon blocks: 12 x 44 = 528
      Active move floats: 60 (4 moves x 15 floats)
      Active move indices: 4
      Global features: 80
      Self-history: 5
      Total: 697

    Final input to MLP:
      matchup(128) + our_bench_maxmean(256) + opp_team_maxmean(256)
      + global(80) + self_history(5) = 725
    ...
    def __init__(self, observation_space, features_dim=256,
        ...
        # CHANGED: move_floats_per_move=15 (was 12 in v17.3sp)
        self.matchup_head = ActiveMatchupHead(
            pokemon_repr_dim, self.move_embed,
            move_embed_dim=24,
            move_floats_per_move=15,
            output_dim=pokemon_repr_dim,
        )

        self.embed_dropout = nn.Dropout(embed_dropout)

        self.n_global_features = 80   # CHANGED: 60 -> 80
        self.n_self_history = 5       # NEW

        # Max+Mean pooling (same as v17.3sp)
        # matchup(128) + our_bench(256) + opp_team(256) + global(80) + self_history(5) = 725
        final_input = (pokemon_repr_dim + pokemon_repr_dim * 2 * 2
                       + self.n_global_features + self.n_self_history)  # 725
    ...
    def forward(self, observations):
        ...
        PER = 44  # Per-pokemon block size (unchanged)
        ...
        # Updated offsets for v18 observation layout
        md_offset = 12 * PER                          # 528: active move floats start
        mi_offset = md_offset + 60                     # 588: active move indices start (was 576)
        gl_offset = mi_offset + 4                      # 592: global features start (was 580)
        sh_offset = gl_offset + self.n_global_features # 672: self-history start

        # CHANGED: 4 moves x 15 floats = 60 (was 4 x 12 = 48)
        move_indices = observations[:, mi_offset:mi_offset+4].long().clamp(
            0, self.config["move_vocab_size"]-1)
        move_floats = observations[:, md_offset:md_offset+60].view(bs, 4, 15)

        matchup = self.matchup_head(our_active, opp_active, move_indices, move_floats)

        # Max+Mean pooling (same as v17.3sp)
        our_bench_max = our_bench.max(dim=1).values
        our_bench_mean = our_bench.mean(dim=1)
        our_bench_pool = torch.cat([our_bench_max, our_bench_mean], dim=-1)

        opp_team = torch.cat([opp_active.unsqueeze(1), opp_bench], dim=1)
        opp_team_max = opp_team.max(dim=1).values
        opp_team_mean = opp_team.mean(dim=1)
        opp_team_pool = torch.cat([opp_team_max, opp_team_mean], dim=-1)

        global_feats = observations[:, gl_offset:gl_offset+self.n_global_features]
        self_history = observations[:, sh_offset:sh_offset+self.n_self_history]

        combined = torch.cat([matchup, our_bench_pool, opp_team_pool,
                              global_feats, self_history], dim=-1)
