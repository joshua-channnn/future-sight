...
class SelfPlayManager:
    ...
    def __init__(
        ...
        # Track total steps for dynamic heuristic probability
        self.total_steps = 0
...
class SelfPlayCallback(BaseCallback):
    ...
    def _on_step(self):
        # Update step count in self-play manager
        self.sp_manager.update_steps(self.num_timesteps)
        
        # === Save checkpoint for self-play pool ===
        if self.num_timesteps % self.checkpoint_freq == 0 and self.num_timesteps > 0:
            ...
            # Add to self-play pool
            self.sp_manager.add_checkpoint(ckpt_path, vec_path)

            # Save resumable state + latest snapshot
...
def train():
    ...
    # Load v13's normalization stats
    ...
    if os.path.exists(RESUME_PATH):
        ...
        # Adjust LR schedule for remaining portion
