# monitor_training.py
"""
Training monitor for Pokemon RL Bot.

Updated for v17 architecture:
- Wang-inspired hyperparameters (gamma=0.9999, gae_lambda=0.754, clip_range=0.08)
- Shared per-Pokemon encoder with ~1.4M params
- 30M step training target
- Aggressive LR decay (5e-4 → 1e-5 via Wang schedule)

Usage:
    python monitor_training.py                    # Single report
    python monitor_training.py --watch            # Continuous monitoring
    python monitor_training.py --watch --interval 30
    python monitor_training.py --log-dir ./logs/ppo_v17_1
    python monitor_training.py --compare v13 v17  # Compare runs
"""
...
class TrainingMonitor:
    ...
    # =========================================================================
    # Healthy ranges — tuned for v17's Wang-inspired hyperparameters
    # =========================================================================
    METRIC_RANGES = {
        # === Core PPO metrics ===
        'train/clip_fraction': {
        ...
        # === Embedding norms ===
        # With shared encoder, embeddings get 12x more gradient signal
        # Expect faster growth than v16 but similar final magnitudes
        ...
        # === Training win rate (during training games) ===
        'train/win_rate': {
            'healthy': (0.2, 0.9),
            'warning': (0.05, 0.95),
        ...
        # === Self-play metrics (if using self-play later) ===
    ...
    # =========================================================================
    # Expected milestones for v17 (30M steps, shared encoder, Wang hyperparams)
    #
    # v17 should learn faster than v16 (3x fewer params, shared encoder)
    # but slower than v13 initially (embeddings need warmup)
    # Wang reached 80% at ~40M steps with 78 workers
    # With 1 worker and smaller model, expect:
    # =========================================================================
    MILESTONES = {
        500_000: {
            'eval/win_rate': 0.15,
