# embedding_extractor_v17sp.py
"""
v17.3 Feature Extractor — Max+Mean Pooling for Self-Play.

Changes from v17.2:
- Bench aggregation: max-pool only → concat(max-pool, mean-pool)
  This doubles the bench representation from 128 to 256 each,
  capturing both "best available option" AND "overall team health".
- Final MLP input: 444 → 700  (128 + 256 + 256 + 60)
- Final MLP reinitialized to accommodate larger input

Everything else is IDENTICAL to v17.2:
- Observation size: 640 (unchanged)
- Per-Pokemon block: 44 (13 indices + 31 floats)
- SharedPokemonEncoder: same architecture, same weights
- ActiveMatchupHead: same architecture, same weights
- Embedding tables: same dimensions, same weights

This means v17.2 weights can be warm-started into v17.3:
- All embedding tables transfer directly
- SharedPokemonEncoder transfers directly
- ActiveMatchupHead transfers directly
- Only final_mlp needs random initialization
"""
...
class V17SPFeatureExtractor(BaseFeaturesExtractor):
    ...
    def __init__(self, observation_space, features_dim=256,
        ...
        # === KEY CHANGE: Max+Mean pooling doubles bench representation ===
